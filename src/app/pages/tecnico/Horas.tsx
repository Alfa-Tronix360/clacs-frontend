import { Clock, Play, Pause, Save, Calendar, Loader } from "lucide-react";
...
export default function TecnicoHoras() {
  ...
  useEffect(() => {
    ...
    const intervalo = setInterval(() => {
      if (tsInicio.current !== null) {
        const segundosDestaSessao = Math.floor((Date.now() - tsInicio.current) / 1000);
        setTempoDecorrido(tempoAcumulado.current + segundosDestaSessao);
      }
    }, 1000);
  ...
  useEffect(() => {
    if (location.state?.intervencaoId && intervencoes.length > 0) {
      setIntervencaoSelecionada(location.state.intervencaoId);
    }
  ...
  const toggleCronometro = async () => {
    ...
    if (!cronometroAtivo) {
      ...
      if (primeiroInicio.current === null) {
  ...
  const salvarTempo = async () => {
    ...
    // Calcular totalSegundos directamente dos refs — não depende do estado React
    ...
    if (totalSegundos === 0) {
      mostrarMensagem("erro", "Inicie o cronômetro antes de salvar.");
      return;
    }
    ...
    try {
      ...
      // ✅ CORRIGIDO: snake_case consistente + campo data incluído
      await registrosHorasAPI.criar({
        intervencao_id: intervencaoSelecionada,
        tecnico_id: tecnicoId,
        horas,
        hora_inicio: horaInicio,          // ← snake_case
        hora_fim: horaFim,                // ← snake_case
        data: new Date().toISOString(),   // ← campo data adicionado
        descricao: descricaoTarefa.trim(),
      });
  ...
  // ✅ CORRIGIDO: usar tempoDecorrido (estado React, atualizado pelo setInterval)
  // em vez de calcular dos refs — refs não provocam re-render, logo o botão ficava sempre disabled
  const tempoTotalSegundos = tempoDecorrido;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Cronômetro</h2>
        <div className="space-y-4">
          ...
          <div className="text-center py-8">
            <div className="text-6xl font-bold text-green-600 mb-2 font-mono">
            ...
            {primeiroInicio.current ? (
              <p className="text-sm text-gray-500 mb-6">
                ...
                {cronometroAtivo ? "" : " (Pausado)"}
              </p>
            ) : (
              <p className="text-sm text-gray-400 mb-6">Pronto para iniciar</p>
            )}
            ...
            <div className="flex gap-3 justify-center">
              ...
              >
                {cronometroAtivo ? (
                  ...
                  <><Play className="w-5 h-5" /> {tempoAcumulado.current > 0 ? "Retomar" : "Iniciar"}</>
                )}
              </button>
            ...
            {cronometroAtivo && (
              <p className="text-xs text-gray-400 mt-3">
                Clique em Salvar para finalizar e registar o tempo
              </p>
            )}
          </div>
        </div>
